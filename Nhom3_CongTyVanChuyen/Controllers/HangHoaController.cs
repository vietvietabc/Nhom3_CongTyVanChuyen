...
namespace Nhom3_CongTyVanChuyen.Controllers
{
    ...
    public class HangHoaController : ControllerBase
    {
        ...
        // GET: api/HangHoa/MaHangHoaByTinhChat/{tinhChat}
        [HttpGet("MaHangHoaByTinhChat/{tinhChat}")]
        public async Task<IActionResult> GetMaHangHoaByTinhChat(string tinhChat)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(tinhChat))
                {
                    return BadRequest("Tính chất hàng hóa không được để trống");
                }

                var maHangHoas = await _context.HangHoas
                    .Where(h => h.TinhChatHangHoa == tinhChat)
                    .Select(h => h.MaHangHoa)
                    .ToListAsync();

                return Ok(maHangHoas);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error retrieving MaHangHoaByTinhChat: {ex.Message}");
                return StatusCode(500, $"Internal server error: {ex.Message}");
            }
        }
